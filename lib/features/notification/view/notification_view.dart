...
class _NotificationViewState extends State<NotificationView> {
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      useGradient:
          true, // Must be true so the background slides with the content
      isSafe: false,
      body: Container(
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                AppTextWidget(
                  text:
                      viewModel.noNotificationMessage ?? "No Notifications Yet",
  ...
  Map<String, dynamic> _getNotificationTheme(dynamic notification) {
    final subject = notification.notificationSubject?.toString() ?? "";
    
    // Creator specific styles for reviews
    if (roleId == Constants.creatorUser) {
      ...
      if (notification.isRedeemed == "1") {
        return {"color": AppColor.activeGreen, "icon": Icons.check_circle};
      }
    }
    
    // Dynamic styles based on subject keywords (for both Creator and Business)
    if (subject.contains("New Booking Request") || subject.contains("Booking Request")) {
      return {"color": AppColor.premiumAccent, "icon": Icons.calendar_today_rounded};
    }
    if (subject.contains("Cancelled") || subject.contains("Rejected")) {
      return {"color": Colors.redAccent, "icon": Icons.cancel_rounded};
    }
    if (subject.contains("Confirmed") || subject.contains("Accepted")) {
      return {"color": AppColor.activeGreen, "icon": Icons.check_circle_rounded};
    }
    if (subject.contains("Payment") || subject.contains("Paid") || subject.contains("Received") || subject.contains("💰") || subject.contains("💸")) {
      return {"color": AppColor.activeGreen, "icon": Icons.monetization_on_rounded};
    }
    if (subject.contains("Redeemed") || subject.contains("Redemption") || subject.contains("🛍️")) {
      return {"color": AppColor.premiumAccent, "icon": Icons.shopping_bag_rounded};
    }
    if (subject.contains("Points") || subject.contains("Earned") || subject.contains("🎉")) {
      return {"color": AppColor.orange, "icon": Icons.card_giftcard_rounded};
    }
    if (subject.contains("Transferred") || subject.contains("Transfer")) {
      return {"color": Colors.blue, "icon": Icons.swap_horiz_rounded};
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return TweenAnimationBuilder<double>(
            duration: Duration(
                milliseconds:
                    300 + (index * 50).clamp(0, 500)), // dynamic stagger
              ...
              child: InkWell(
                ...
                onTap: () {
                  if (roleId == Constants.businessUser) {
                    ...
                    final bool isRestaurant = true; // Visits are enabled for all categories
                    final subjectStr = notification.notificationSubject?.toLowerCase() ?? "";
                    final textStr = notification.notificationText?.toLowerCase() ?? "";
                    
                    if (subjectStr.contains("booking") || 
                        subjectStr.contains("book") || 
                        subjectStr.contains("cancel") ||
                        textStr.contains("booking") || 
                        textStr.contains("book") || 
                        textStr.contains("cancel")) {
                      // Redirect to Bookings tab
                      homeVM.changeIndex(isRestaurant ? 2 : 1, false);
                    } else {
                      // Redirect to Wallet tab
                      final int walletIndex = isRestaurant ? 3 : 2;
                      homeVM.changeIndex(walletIndex, false);
                    }
                  }
                },
                child: Padding(
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            AppTextWidget(
                              text:
                                  notification.notificationSubject ?? "Alert",
                              fontSize: 16.sp,
                              fontWeight: FontWeight.w800,
                              color: AppColor.premiumTextPrimary,
                            ),
                                ...
                                children: [
                                  ...
                                  AppTextWidget(
                                    text: notification.createdAt!
                                            .toString()
                                            .split('.')[
                                        0], // Format to exclude milliseconds
                                    fontSize: 11.sp,
                                    color: AppColor.grey,
                                    fontWeight: FontWeight.w600,
                                  ),

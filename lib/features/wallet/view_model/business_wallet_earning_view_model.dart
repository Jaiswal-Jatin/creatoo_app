...
class BusinessWalletEarningViewModel with ChangeNotifier {
  final BusinessEarningWalletRepository _myRepo = BusinessEarningWalletRepository();
  ...
  ApiResponse<BusinessWalletTransactionResponse> transactionResponse = ApiResponse.loading();
  ...
  Map<DateTime, List<Transaction>> get groupedTransactions {
    if (transactionResponse.data?.data?.transactions == null) return {};
    
    Map<DateTime, List<Transaction>> grouped = {};
    for (var transaction in transactionResponse.data!.data!.transactions!) {
  ...
  Future<void> fetchBusinessWalletTransactions() async {
    ...
    response.fold(
      ...
      (r) async {
        walletBalance = (r.data?.businessWallet ?? 0.0);
